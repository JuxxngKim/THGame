...
(`Logic/PlayerWorkExecutor.cs`)가 담당한다 — 도메인 핸들러(어떤 패킷을 처리하느냐)와 변경 축이 다른
...
| `Logic/OutGameLogicEventor.cs` | OutGame 도메인 Eventor. OutGame 핸들러 + 주기 `Event`. `Work` 는 executor 로 위임 |
| `Logic/PlayerWorkExecutor.cs` | worker phase 오케스트레이션. PlayerArchive 소유 + Player 단위 병렬 분산(`Run`) + lifecycle |
...
| `Logic/PlayerArchive.cs` | 세션 단위 Player 보관자. `Players` 로 전체 순회. PlayerWorkExecutor 가 소유 |
